...
describe('Forecast API Logic', () => {
  ...
  const simulateForecastAPI = async (lat: string | null, lon: string | null) => {
    if (!lat || !lon) {
      return { 
  ...
  it('returns forecast data for valid coordinates', async () => {
    ...
    expect(result.data?.periods).toEqual(mockForecastResponse.properties.periods)
  })
  ...
  it('makes correct requests to weather.gov API', async () => {
    ...
    expect(mockForecastFetch).toHaveBeenNthCalledWith(1, 'https://api.weather.gov/points/40.748817,-73.985428', {
      headers: {
        'User-Agent': 'Geo-Weather (geo-weather-gt@gmail.com)'
      }
    })
    expect(mockForecastFetch).toHaveBeenNthCalledWith(2, 'https://api.weather.gov/gridpoints/OKX/33,37/forecast', {
      headers: {
        'User-Agent': 'Geo-Weather (geo-weather-gt@gmail.com)'
      }
    })
  ...
  it('handles decimal coordinates', async () => {
    ...
    expect(mockForecastFetch).toHaveBeenNthCalledWith(1, 'https://api.weather.gov/points/40.123456,-73.987654', {
      headers: {
        'User-Agent': 'Geo-Weather (geo-weather-gt@gmail.com)'
      }
    })
