  ...
  describe("getCoordinatesFromAddress", () => {
    ...
    it("makes request to correct API endpoint", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "/api/geocode?address=123%20Main%20St%2C%20Anytown%2C%20USA"
      );
    });
    ...
    it("properly encodes address in URL", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "/api/geocode?address=123%20Main%20St%20%26%20Oak%20Ave%2C%20New%20York%2C%20NY"
      );
    });
    ...
    it("throws error when fetch fails", async () => {
      ...
      await expect(
        getCoordinatesFromAddress("invalid address")
    ...
    it("throws error when no address matches found", async () => {
      ...
      await expect(
        getCoordinatesFromAddress("nonexistent address")
