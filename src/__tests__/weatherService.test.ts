...
describe('weatherService', () => {
  ...
  describe('getWeekForecast', () => {
    const mockForecastPeriods: ForecastPeriod[] = Array.from({ length: 20 }, (_, i) => ({
      number: i + 1,
      name: `Day ${i + 1}`,
    ...
    it('makes request to correct API endpoint', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/forecast?lat=40.748817&lon=-73.985428')
    })
    ...
    it('handles decimal coordinates correctly', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/forecast?lat=40.123456&lon=-73.987654')
    })
    ...
    it('throws error when fetch fails', async () => {
      ...
      await expect(getWeekForecast(40.748817, -73.985428))
        .rejects
        .toThrow('Forecast data unavailable')
    })
    ...
    it('throws error with generic message when no error message provided', async () => {
      ...
      await expect(getWeekForecast(40.748817, -73.985428))
        .rejects
        .toThrow('Failed to load forecast')
    })
